...
# Capture the build to a log and surface it on failure. Running under bare
# --quiet with `set -e` aborts the script before any diagnostic prints, leaving
# an empty test.log when the build fails.
echo "Building image with --workdir /workspace..."
BUILD_LOG="workdir_build.log"
set +e
...
if [ "$EXIT_CODE" -ne 0 ]; then
    echo "Error: --workdir build failed (exit $EXIT_CODE)."
    cat "$BUILD_LOG"
    rm -f "$BUILD_LOG"
    exit "$EXIT_CODE"
fi
